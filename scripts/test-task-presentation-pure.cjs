...
const { greetingForHour, groupAssignments, isCountTask, countTaskActionLabel }=cjs.exports;
...
const tasks=[
 ...
 {id:'skipped',title:'Skipped work',status:'SKIPPED',scheduledDate:'2026-08-28T00:00:00Z',priority:'NORMAL',dueAt:null},
];
...
assert.deepEqual(JSON.parse(JSON.stringify(grouped.skippedToday.map(x=>x.id))),['skipped']);
assert.equal(isCountTask({title:'Complete Store Scan',instructions:null}),true);
assert.equal(countTaskActionLabel({title:'Complete Store Scan',instructions:null}),'Start Count');
assert.equal(countTaskActionLabel({title:'Cycle count aisle 4',instructions:null}),'Start Count');
assert.equal(isCountTask({title:'Opening safety walk',instructions:'Check exits'}),false);
