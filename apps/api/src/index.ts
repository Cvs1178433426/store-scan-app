...
import { phoneRecoveryRoutes } from "./routes/phoneRecovery.js";
...
import { startPendingRegistrationCleanupJob } from "./jobs/pendingRegistrationCleanup.js";
import { startPhoneRecoveryCleanupJob } from "./jobs/phoneRecoveryCleanup.js";
import { startExpiredMfaChallengeCleanupJob, startSmsVerificationDispatchJob } from "./jobs/smsVerificationDispatch.js";
...
import { assertSmsMfaConfig, assertTwilioRestrictedKeyPolicies } from "./lib/verificationProvider.js";
import { isSmsEnrollmentRequired } from "./lib/smsMigration.js";
import { assertSmsMfaMigrationState } from "./lib/smsMfaMigrationState.js";
...
export async function buildApp() {
  ...
  await app.register(phoneRecoveryRoutes, { prefix: "/api/auth" });
...
export async function startServer() {
  await assertSmsMfaMigrationState();
  await assertTwilioRestrictedKeyPolicies();
  const app = await buildApp();
  startExpiredMfaChallengeCleanupJob();
  if (process.env.SMS_MFA_ENABLED === "true") {
    startPendingRegistrationCleanupJob();
    startPhoneRecoveryCleanupJob();
    startSmsVerificationDispatchJob();
  }
