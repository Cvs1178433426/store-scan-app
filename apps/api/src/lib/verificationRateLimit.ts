...
function configuredLimit(name: string, fallback: number): number {
  ...
  if (!/^\d+$/.test(raw)) throw new VerificationRateLimitUnavailableError();
  const value = Number(raw);
  if (!Number.isSafeInteger(value) || value < 1 || value > 10_000) throw new VerificationRateLimitUnavailableError();
  return value;
}

export function verificationPhonePrefixHash(phoneE164: string): string {
  if (!/^\+1[2-9]\d{9}$/.test(phoneE164)) throw new VerificationRateLimitUnavailableError();
  const key = process.env.RATE_LIMIT_HMAC_KEY?.trim();
  if (!key) throw new VerificationRateLimitUnavailableError();
  return createHmac("sha256", key).update(phoneE164.slice(0, 5)).digest("hex");
}
...
type RateLimitKey = { dimension: "phone" | "phone-prefix" | "account" | "ip" | "global"; keyHash: string };
...
function bucketDefinitions(action: string, keys: RateLimitKey[]): RateLimitBucket[] {
  ...
  const directKeys = subjectKeys.filter(({ dimension }) => dimension !== "phone-prefix");
  const prefixKeys = subjectKeys.filter(({ dimension }) => dimension === "phone-prefix");
  const cooldownKeys = directKeys.filter(({ dimension }) => dimension !== "ip");
  return [
    ...cooldownKeys.map((key) => ({ ...key, action: `sms-send:cooldown:${action}`, limit: 1, durationSeconds: COOLDOWN_SECONDS, cooldown: true })),
    ...directKeys.map((key) => ({ ...key, action: "sms-send:15m", limit: 3, durationSeconds: SHORT_WINDOW_SECONDS, cooldown: false })),
    ...directKeys.map((key) => ({ ...key, action: "sms-send:24h", limit: 10, durationSeconds: DAILY_WINDOW_SECONDS, cooldown: false })),
    ...prefixKeys.map((key) => ({ ...key, action: "sms-send:prefix:15m", limit: 20, durationSeconds: SHORT_WINDOW_SECONDS, cooldown: false })),
    ...prefixKeys.map((key) => ({ ...key, action: "sms-send:prefix:24h", limit: 100, durationSeconds: DAILY_WINDOW_SECONDS, cooldown: false })),
    ...(keys.some(({ dimension }) => dimension === "global")
      ? [{
        dimension: "global" as const,
        keyHash: "global",
        action: "sms-send:global:1m",
        limit: configuredLimit("SMS_GLOBAL_SEND_LIMIT_PER_MINUTE", 100),
        durationSeconds: GLOBAL_WINDOW_SECONDS,
        cooldown: false,
      }]
      : []),
  ];
...
function suppliedKeys(input: Omit<VerificationBudgetInput, "now" | "reservation">): RateLimitKey[] {
  const supplied = [
    ...
    ["phone-prefix", input.phonePrefixHash],
...
export function createSmsDispatchBudgetConsumer(store: VerificationRateLimitStore) {
  return async (now: Date): Promise<boolean> => {
    if (Number.isNaN(now.getTime())) throw new VerificationRateLimitUnavailableError();
    try {
      const globalKey = { dimension: "global" as const, keyHash: "global" };
      const result = await store.consumeAtomic({
        tenantScope: TENANT_SCOPE,
        action: "SMS_PROVIDER_DISPATCH",
        keys: [globalKey],
        buckets: [{
          ...globalKey,
          action: "sms-provider-dispatch:global:1m",
          limit: configuredLimit("SMS_GLOBAL_SEND_LIMIT_PER_MINUTE", 100),
          durationSeconds: GLOBAL_WINDOW_SECONDS,
          cooldown: true,
        }],
...
function normalizedAttemptInput(input: VerificationAttemptInput, consume: boolean): AtomicConsumption {
  if (!input.action.trim() || Number.isNaN(input.now.getTime())) throw new Error("Invalid verification attempt input.");
  const action = input.action.trim().toUpperCase();
  const keys = suppliedKeys(input);
  return {
    tenantScope: TENANT_SCOPE,
    action,
    keys,
    buckets: consume
      ? keys.map((key) => ({
        ...key,
        action: `otp-check:15m:${action}`,
        // The store opens a lock when an existing count reaches its limit, so
        // four recorded misses make the fifth submission the locking attempt.
        limit: MAX_INCORRECT_ATTEMPTS - 1,
        durationSeconds: SHORT_WINDOW_SECONDS,
        cooldown: false,
      }))
      : [],
    now: input.now,
...
function createAttemptConsumer(store: VerificationRateLimitStore, consume: boolean) {
  return async (input: VerificationAttemptInput): Promise<VerificationBudgetResult> => {
    try {
      return await store.consumeAtomic(normalizedAttemptInput(input, consume));
    } catch (error) {
      if (error instanceof VerificationRateLimitUnavailableError) throw error;
      throw new VerificationRateLimitUnavailableError({ cause: error });
    }
...
export function createIncorrectVerificationAttemptConsumer(store: VerificationRateLimitStore) {
  return createAttemptConsumer(store, true);
}

export function createVerificationAttemptLockChecker(store: VerificationRateLimitStore) {
  return createAttemptConsumer(store, false);
}
...
const consumeDispatchWithPrisma = createSmsDispatchBudgetConsumer(new PrismaVerificationRateLimitStore());
const attemptStoreWithPrisma = new PrismaVerificationRateLimitStore();
const consumeIncorrectAttemptWithPrisma = createIncorrectVerificationAttemptConsumer(attemptStoreWithPrisma);
const checkAttemptLockWithPrisma = createVerificationAttemptLockChecker(attemptStoreWithPrisma);
...
export async function consumeSmsDispatchBudget(now: Date): Promise<boolean> {
  return consumeDispatchWithPrisma(now);
}

export async function consumeIncorrectVerificationAttempt(input: VerificationAttemptInput): Promise<VerificationBudgetResult> {
  return consumeIncorrectAttemptWithPrisma(input);
}

export async function checkVerificationAttemptLock(input: VerificationAttemptInput): Promise<VerificationBudgetResult> {
  return checkAttemptLockWithPrisma(input);
}
