...
import {
  ...
  createSmsDispatchBudgetConsumer,
    ...
    const tx = {
      $executeRaw: async (strings: TemplateStringsArray, ...values: unknown[]) => {
        ...
        if (sql.startsWith('INSERT INTO "VerificationRateLimit"')) {
          ...
          const isLock = sql.includes('"expiresAt" = GREATEST');
          const [id, tenantScope, action, keyHash, windowStart] = values as [string, string, string, string, Date];
          const count = isLock ? 0 : 1;
          const limit = isLock ? 0 : values[5] as number;
          const expiresAt = values[isLock ? 5 : 6] as Date;
          const updatedAt = values[isLock ? 6 : 7] as Date;
...
describe("durable verification budgets", () => {
  it("locks a verification subject for fifteen minutes on the fifth incorrect submission", async () => {
    const consume = createIncorrectVerificationAttemptConsumer(new InMemoryVerificationRateLimitStore());
    const attempt = (seconds: number) => ({
      action: "LOGIN",
      phoneHash: key("+16317423355"),
      accountHash: key("user-1"),
      now: at(seconds),
    ...
    for (let number = 1; number <= 4; number += 1) {
      await expect(consume(attempt(number))).resolves.toEqual({ allowed: true, retryAfterSeconds: 0 });
    }
    await expect(consume(attempt(5))).resolves.toEqual({ allowed: false, retryAfterSeconds: 900 });
    await expect(consume(attempt(6))).resolves.toEqual({ allowed: false, retryAfterSeconds: 899 });
  });
  ...
  it("uses the configured global send limit", async () => {
    process.env.SMS_GLOBAL_SEND_LIMIT_PER_MINUTE = "2";
    try {
      const consume = createVerificationBudgetConsumer(new InMemoryVerificationRateLimitStore());
      await expect(consume(input({ phoneHash: key("phone-a"), accountHash: undefined, ipHash: undefined }))).resolves.toEqual({ allowed: true, retryAfterSeconds: 0 });
      await expect(consume(input({ phoneHash: key("phone-b"), accountHash: undefined, ipHash: undefined }))).resolves.toEqual({ allowed: true, retryAfterSeconds: 0 });
      await expect(consume(input({ phoneHash: key("phone-c"), accountHash: undefined, ipHash: undefined }))).resolves.toEqual({ allowed: false, retryAfterSeconds: 900 });
    } finally {
      delete process.env.SMS_GLOBAL_SEND_LIMIT_PER_MINUTE;
    }
  });

  it("limits aggregate sends to different phones sharing one numbering prefix", async () => {
    const consume = createVerificationBudgetConsumer(new InMemoryVerificationRateLimitStore());
    const phonePrefixHash = key("+1631");
    for (let attempt = 0; attempt < 20; attempt += 1) {
      await expect(consume(input({
        phoneHash: undefined, accountHash: undefined, ipHash: undefined,
        phonePrefixHash, now: at(attempt * 31),
      }))).resolves.toEqual({ allowed: true, retryAfterSeconds: 0 });
    }
    await expect(consume(input({
      phoneHash: undefined, accountHash: undefined, ipHash: undefined,
      phonePrefixHash, now: at(20 * 31),
    }))).resolves.toEqual({ allowed: false, retryAfterSeconds: 900 });
  });
  ...
  it("enforces the configured global ceiling again when SMS reaches the provider", async () => {
    const original = process.env.SMS_GLOBAL_SEND_LIMIT_PER_MINUTE;
    process.env.SMS_GLOBAL_SEND_LIMIT_PER_MINUTE = "2";
    try {
      const consume = createSmsDispatchBudgetConsumer(new InMemoryVerificationRateLimitStore());
      await expect(consume(at(0))).resolves.toBe(true);
      await expect(consume(at(1))).resolves.toBe(true);
      await expect(consume(at(2))).resolves.toBe(false);
      await expect(consume(at(61))).resolves.toBe(true);
    } finally {
      if (original === undefined) delete process.env.SMS_GLOBAL_SEND_LIMIT_PER_MINUTE;
      else process.env.SMS_GLOBAL_SEND_LIMIT_PER_MINUTE = original;
    }
  });
  ...
  it("shares a fifth-incorrect-attempt lock with production SMS send budgets", async () => {
    const database = new TransactionDatabase();
    const store = new PrismaVerificationRateLimitStore(database);
    const consumeAttempt = createIncorrectVerificationAttemptConsumer(store);
    const consumeSend = createVerificationBudgetConsumer(store);
    const attempt = (seconds: number) => ({
      action: "LOGIN",
      phoneHash: key("+16317423355"),
      accountHash: key("user-1"),
    ...
    for (let number = 1; number <= 4; number += 1) {
      await expect(consumeAttempt(attempt(number))).resolves.toEqual({ allowed: true, retryAfterSeconds: 0 });
    }
    await expect(consumeAttempt(attempt(5))).resolves.toEqual({ allowed: false, retryAfterSeconds: 900 });
    await expect(consumeSend(input({ now: at(6) }))).resolves.toEqual({ allowed: false, retryAfterSeconds: 899 });
  });
