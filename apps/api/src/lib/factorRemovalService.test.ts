...
function dependencies() {
  ...
  const policy: FactorRemovalChallengePolicy = {
    ...
    completeChallenge: vi.fn(async (input, completeAtomically) => ({
      approved: true,
      value: await completeAtomically(input.challengeId, new Date("2026-09-01T12:00:00Z"), input.tokenVersionAtIssue ?? null),
    })),
  };
...
describe("TOTP factor removal", () => {
  ...
  it("starts only an SMS FACTOR_REMOVAL challenge", async () => {
    ...
    expect(policy.startChallenge).toHaveBeenCalledWith({
      ...
      dimensions: ["account:a", "ip:b", expect.stringMatching(/^phone-prefix:[a-f0-9]{64}$/)],
    });
  });
...
describe("Prisma factor-removal repository", () => {
  ...
  beforeEach(() => {
    ...
    mocks.queryRaw.mockResolvedValue([{ id: "user-1", mfaEnabled: true, tokenVersion: 3 }]);
  ...
  it("atomically consumes the bound SMS challenge, removes TOTP, rotates sessions, and audits", async () => {
    ...
    expect(normalizedSql(mocks.queryRaw.mock.calls[0])).toBe(
      'SELECT "id", "mfaEnabled", "tokenVersion" FROM "User" WHERE "id" = ? FOR UPDATE',
    );
  ...
  it("rejects factor removal when the account token version changed after challenge issuance", async () => {
    mocks.queryRaw.mockResolvedValue([{ id: "user-1", mfaEnabled: true, tokenVersion: 4 }]);
    const repository = new PrismaFactorRemovalRepository();

    await expect(repository.removeTotpFromChallenge(removalInput))
      .rejects.toBeInstanceOf(FactorRemovalVerificationRejectedError);
