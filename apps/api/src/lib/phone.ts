...
export type PhoneLookupCandidate = { version: number; hash: string };
...
type PhoneKeySetting = "PHONE_ENCRYPTION_KEYS" | "PHONE_LOOKUP_HMAC_KEYS";

export function parsePhoneKeyRing(name: PhoneKeySetting, rawValue: string | undefined): VersionedKey[] {
  if (!rawValue?.trim()) throw new Error(`${name} is required.`);
  const values = rawValue.split(",").map((value) => value.trim());
  if (values.some((value) => !value)) throw new Error(`${name} must not contain empty key-ring entries.`);
  const versions = new Set<number>();
  return values.map((value) => {
    const match = /^(\d+):([a-fA-F0-9]{64})$/.exec(value);
    if (!match) throw new Error(`${name} entries must use version:32-byte-hex format.`);
    const version = Number(match[1]);
    if (!Number.isSafeInteger(version) || version < 1) {
      throw new Error(`${name} key versions must be a safe positive integer.`);
    }
    if (version > 2_147_483_647) {
      throw new Error(`${name} key versions must fit a positive signed 32-bit integer.`);
    }
    if (versions.has(version)) throw new Error(`${name} contains a duplicate key version.`);
    versions.add(version);
    return { version, key: Buffer.from(match[2], "hex") };
  });
}

function keyRing(name: PhoneKeySetting): VersionedKey[] {
  return parsePhoneKeyRing(name, process.env[name]);
}
...
export function hashPhoneCandidates(e164: string): PhoneLookupCandidate[] {
  return keyRing("PHONE_LOOKUP_HMAC_KEYS").map(({ version, key }) => ({
    version,
    hash: createHmac("sha256", key).update(e164, "utf8").digest("hex"),
  }));
}
