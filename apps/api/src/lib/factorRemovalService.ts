...
import { verificationPhonePrefixHash } from "./verificationRateLimit.js";
...
export class FactorRemovalService {
  ...
  async startTotpRemoval(
    ...
    const input: StartChallengeInput = {
      ...
      dimensions: [...dimensions, `phone-prefix:${verificationPhonePrefixHash(phone.phoneE164)}`],
    };
  ...
  async confirmTotpRemoval(
    ...
    try {
      result = await this.policy.completeChallenge({
        ...
        tokenVersionAtIssue: user.tokenVersion,
        code,
      }, (boundChallengeId, approvedAt, tokenVersionAtIssue) => this.repository.removeTotpFromChallenge({
        challengeId: boundChallengeId,
        userId: user.id,
        approvedAt,
        correlationId,
        tokenVersionAtIssue: tokenVersionAtIssue ?? -1,
      }));
    } catch (error) {
...
export class PrismaFactorRemovalRepository implements FactorRemovalRepository {
  async removeTotpFromChallenge(input: RemoveTotpInput): Promise<boolean> {
    return prisma.$transaction(async (tx) => {
      const users = await tx.$queryRaw<Array<{ id: string; mfaEnabled: boolean; tokenVersion: number }>>`
        SELECT "id", "mfaEnabled", "tokenVersion" FROM "User" WHERE "id" = ${input.userId} FOR UPDATE
      `;
      if (!users[0]?.mfaEnabled || users[0].tokenVersion !== input.tokenVersionAtIssue) throw new FactorRemovalVerificationRejectedError();
      ...
      const consumed = await tx.$executeRaw`
        ...
        WHERE "id" = ${input.challengeId} AND "userId" = ${input.userId}
          ...
          AND "tokenVersionAtIssue" = ${input.tokenVersionAtIssue}
