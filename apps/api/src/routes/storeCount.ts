...
export async function storeCountRoutes(app: FastifyInstance) {
  ...
  app.post("/sessions/:id/scan", async (request, reply) => {
    ...
    const session = await prisma.storeCountSession.findUnique({ where: { id } });
    if (!session) return reply.code(404).send({ error: "count session not found" });
    if (session.status !== "ACTIVE") return reply.code(409).send({ error: "count session is not active" });

    const { barcodeValue, locationCode, quantityDelta } = parsed.data;
    ...
    const existing = await prisma.storeCountEntry.findUnique({
      where: {
    ...
    const entry = existing
      ? await prisma.storeCountEntry.update({
          where: { id: existing.id },
          data: {
            quantity: { increment: quantityDelta },
            itemId: itemId ?? existing.itemId,
            productName: productName ?? existing.productName,
            packageSize: packageSize ?? existing.packageSize,
            scannedAt: new Date(),
          },
        })
      : await prisma.storeCountEntry.create({
          data: {
            sessionId: id,
            itemId,
            barcodeValue,
  ...
  app.get("/sessions/:id/summary", async (request, reply) => {
    ...
    for (const entry of session.entries) {
      let row = byBarcode.get(entry.barcodeValue);
      if (!row) {
        row = {
  ...
  app.post("/sessions/:id/complete", async (request, reply) => {
    const { id } = request.params as { id: string };
    const existing = await prisma.storeCountSession.findUnique({ where: { id } });
