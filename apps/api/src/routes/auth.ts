...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/bootstrap/admin", async (request, reply) => {
    ...
    const user = await prisma.user.create({
      data: { name, email, employeeNumber, passwordHash, role: "ADMIN" },
    });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const [passwordHash, recoveryPinHash, employeeNumber] = await Promise.all([
        ...
        bcrypt.hash(recoveryPin, 10),
        createEmployeeNumber(),
      ]);
      const user = await prisma.user.create({
        data: { name, email, employeeNumber, passwordHash, recoveryPinHash, role: "GENERAL" },
      });
      ...
      const user = await prisma.user.findFirst({
        where: {
          OR: [
            ...
            { employeeNumber: identifier.toUpperCase() },
          ],
        },
      });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!user || !user.isActive || !user.recoveryPinHash) {
        return reply.code(401).send({ error: "We could not verify that account." });
      }
  ...
  app.get("/users", { preHandler: [app.authenticate, app.requireAdmin] }, async () => {
    return prisma.user.findMany({
      select: { id: true, name: true, email: true, employeeNumber: true, role: true, isActive: true },
