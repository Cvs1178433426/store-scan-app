...
import { OneTimeSecrets } from "../../components/OneTimeSecrets";
import { VerificationCodeForm } from "../../components/VerificationCodeForm";
import { ApiError, apiJson } from "../../lib/api";
import { createAuthFlow, formatWaitTime, reduceAuthFlow } from "../../lib/authFlow";
import { registrationDeliveryHelp } from "../../lib/registrationCopy";
...
type RegistrationResult = { token: string; backupCodes: string[] };
...
export default function RegisterPage() {
  ...
  async function submitDetails(event: FormEvent) {
    ...
    try {
      ...
      if (caught instanceof ApiError && caught.status === 429) {
        ...
        setError("Check your name, email address, mobile number, and password, then try again.");
      } else {
  ...
  if (flow.step === "complete" && recoveryCodes.length > 0) {
    return (
      <OneTimeSecrets
        title="Save your recovery codes"
        hint="Each code works once if you cannot receive a text message. They will not be shown again."
        secrets={recoveryCodes.map((value, index) => ({ label: `Recovery code ${index + 1}`, value }))}
        downloadFilename="continuixai-recovery-codes.txt"
    ...
    return (
      <main className="auth-page">
        <section className="auth-card" aria-labelledby="registration-code-title">
          <BrandLockup />
          <h1 id="registration-code-title">Enter your verification code</h1>
          <p className="auth-intro">{registrationDeliveryHelp(flow.maskedDestination ?? "your phone")} Keep this screen open. We never ask you to scan a QR code during registration.</p>
