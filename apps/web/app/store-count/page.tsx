...
export default function StoreCountPage() {
  ...
  async function refreshSession(id: string) {
    try {
      setSession(await apiJson<CountSession>(`/api/store-count/sessions/${id}`));
    } catch {
      // best-effort; the optimistic scan result is already visible
    }
  }
  ...
  async function handleBarcode(rawValue: string, quantityDelta = 1) {
    ...
    try {
      ...
      setSession((current) => {
        ...
        return { ...current, entries: [entry, ...current.entries.filter((existing) => existing.id !== entry.id)] };
      });
      setFlash({
  ...
  async function toggleTorch() {
    ...
    try {
      ...
      show("Flash is not available on this device.", "error");
    }
  }
  ...
  async function finishSession() {
    ...
    try {
      ...
      show(error instanceof Error ? error.message : "Could not finish count.", "error");
    }
  }
    ...
    <main className="container" style={{ maxWidth: 680, paddingBottom: 48 }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "end", marginBottom: 14, gap: 12 }}>
        <div>
          ...
          <h1 style={{ margin: "3px 0 0" }}>Count</h1>
        </div>
      ...
      {!session && (
        <section className="card" style={{ padding: 24, textAlign: "center" }}>
          <h2 style={{ marginTop: 0 }}>Ready to count?</h2>
          <p style={{ opacity: 0.75 }}>Start a session, pick a location, then scan continuously.</p>
          <button type="button" onClick={() => void startSession()} style={{ minHeight: 52, width: "100%" }}>Start Count</button>
        </section>
      )}
        ...
        <>
          ...
          {session.status === "ACTIVE" && (
            <>
              ...
              <div style={{ textAlign: "center", margin: "10px 0 12px", minHeight: 42, fontWeight: flash ? 800 : 500 }}>
              ...
              {entriesHere.length > 0 && (
                <section style={{ marginTop: 18 }}>
                  <h2 style={{ fontSize: 14 }}>Counted here</h2>
                  <div style={{ display: "grid", gap: 6 }}>
                    {entriesHere.slice(0, 20).map((entry) => <div key={entry.id} className="card" style={{ padding: 10, display: "flex", justifyContent: "space-between", gap: 12 }}><span>{entry.product?.name ?? entry.barcodeValue}</span><strong>{entry.quantity}</strong></div>)}
                  </div>
                </section>
              )}
