...
export default function LoginPage() {
  ...
  if (stage === "setup") return (
    <AuthShell>
      ...
      <form onSubmit={confirmEnrollment} className="form" style={{ marginTop: 18 }}>
        <input style={fieldStyle} inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]*" maxLength={6} placeholder="6-digit verification code" value={mfaCode} onChange={(e) => setMfaCode(e.target.value.replace(/\D/g, "").slice(0, 6))} required />
  ...
  if (stage === "verify") return (
    <AuthShell>
      <h1 style={brandStyle}>Multi-Factor Verification</h1>
      <p style={helperStyle}>Enter the 6-digit code from your authenticator app, or use a backup code.</p>
      <form onSubmit={verifyMfa} className="form">
        <input style={fieldStyle} autoFocus autoComplete="one-time-code" placeholder="6-digit code or backup code" value={mfaCode} onChange={(e) => setMfaCode(e.target.value.toUpperCase())} required />
        <button type="submit" disabled={loading}>{loading ? "Verifying..." : "Verify & Sign In"}</button>
        {error && <p className="error-text">{error}</p>}
      </form>
      <button type="button" className="secondary" onClick={() => { setStage("password"); setMfaCode(""); setPassword(""); }} style={{ marginTop: 12, width: "100%" }}>Back to Sign In</button>
    </AuthShell>
  );

  if (stage === "backup") return (
    <AuthShell>
      <h1 style={brandStyle}>MFA Is Enabled</h1>
      <p style={helperStyle}><strong>Save these backup codes now.</strong> Each code can be used once if you lose access to your authenticator app.</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, margin: "18px 0" }}>{backupCodes.map((code) => <code key={code} style={{ fontSize: 14, padding: 8, background: "var(--color-surface-hover)", borderRadius: 8 }}>{code}</code>)}</div>
      <button type="button" onClick={finishEnrollment} style={{ width: "100%" }}>I Saved My Backup Codes — Continue</button>
    </AuthShell>
  );
    ...
    <AuthShell>
      ...
      {needsBootstrap ? (
        <>
          ...
          <p style={helperStyle}>Create the first administrator account. Multi-factor authentication will be required immediately after setup.</p>
          <form onSubmit={handleBootstrapSubmit} className="form">
            <input style={fieldStyle} type="text" autoComplete="name" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} required />
            <input style={fieldStyle} type="email" inputMode="email" autoComplete="email" placeholder="Email" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required />
            <input style={fieldStyle} type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required />
            <input style={fieldStyle} type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="Confirm password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
            <label style={showStyle}><input type="checkbox" checked={showPassword} onChange={(e) => setShowPassword(e.target.checked)} />Show password</label>
        ...
        <>
          ...
          <p style={helperStyle}>Enter your email address or Employee Number. You’ll verify with MFA next.</p>
          <form onSubmit={handleSubmit} className="form">
            <input style={fieldStyle} type="text" autoCapitalize="none" autoComplete="username" placeholder="Email or Employee Number" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required />
            <input style={fieldStyle} type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required />
            <label style={showStyle}><input type="checkbox" checked={showPassword} onChange={(e) => setShowPassword(e.target.checked)} />Show password</label>
            <button type="submit" disabled={loading}>{loading ? "Signing in..." : "Sign In"}</button>
            {error && <p className="error-text" style={{ margin: "2px 0 0" }}>{error}</p>}
          </form>
          <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--color-border)", display: "grid", gap: 9, fontSize: 14 }}>
