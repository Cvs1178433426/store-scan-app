...
export function normalizeVerificationCode(value: string, method: VerificationMethod, maxLength?: number): string {
  const limit = maxLength ?? (method === "RECOVERY_CODE" ? 10 : 6);
  if (method === "RECOVERY_CODE") return value.replace(/[^a-z0-9]/gi, "").toUpperCase().slice(0, limit);
