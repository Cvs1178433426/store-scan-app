...
export function VerificationCodeForm({
  ...
  const requiredLength = codeLength ?? (method === "RECOVERY_CODE" ? 10 : 6);
  ...
  function updateCode(value: string) {
    setCode(normalizeVerificationCode(value, method, requiredLength));
  }
  ...
  return (
    <form className="form auth-code-form" onSubmit={submit} noValidate>
      <div className="auth-field">
        <label htmlFor={inputId}>{codeLabel ?? METHOD_COPY[method].label}</label>
        <input
          id={inputId}
          autoFocus
          type="text"
          ...
          pattern={method === "RECOVERY_CODE" ? `[A-Fa-f0-9]{${requiredLength}}` : `[0-9]{${requiredLength}}`}
        ...
        <p className="auth-help" id={helpId}>
          {method === "SMS" && maskedDestination
            ? `A code was requested for ${maskedDestination}. `
            : ""}
          {codeHelp ?? METHOD_COPY[method].help}
        </p>
        {method === "SMS" && !codeHelp && (
          <p className="auth-help">If you requested another code recently, more than one text may arrive. Use the newest code.</p>
        )}
      </div>
